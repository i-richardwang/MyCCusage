...
program
  ...
  .action(async (options) => {
    ...
    if (options.dryRun) {
      console.log(`🧪 Dry run mode: collecting data for ${formatAgentList(agents)}`)
      try {
...
program
  ...
  .action(async () => {
    ...
    try {
      ...
      if (allData.length === 0) {
        console.log(`⚠️  No usage data found for any agent. Make sure the tools are installed.`)
      } else {
        const totalRecords = allData.reduce((sum, d) => sum + d.daily.length, 0)
        console.log(`✅ Found ${totalRecords} total daily records across ${allData.length} agent(s)`)
      }
