...
async function getUnifiedUsageStats() {
  ...
  const statsQuery = db.$with("usage_stats").as(
    db
      .select({
        totalCost: sql<number>`SUM(${usageRecords.totalCost})::numeric`.as(
          "total_cost",
        ),
        totalTokens: sql<number>`SUM(${usageRecords.totalTokens})::bigint`.as(
          "total_tokens",
        ),
        totalInputTokens:
          sql<number>`SUM(${usageRecords.inputTokens})::bigint`.as(
            "total_input_tokens",
          ),
        totalOutputTokens:
          sql<number>`SUM(${usageRecords.outputTokens})::bigint`.as(
            "total_output_tokens",
          ),
        totalCacheCreationTokens:
          sql<number>`SUM(${usageRecords.cacheCreationTokens})::bigint`.as(
            "total_cache_creation_tokens",
          ),
        totalCacheReadTokens:
          sql<number>`SUM(${usageRecords.cacheReadTokens})::bigint`.as(
            "total_cache_read_tokens",
          ),
        totalCredits: sql<number>`SUM(${usageRecords.credits})::numeric`.as(
          "total_credits",
        ),
        activeDays:
          sql<number>`COUNT(DISTINCT ${usageRecords.date})::bigint`.as(
            "active_days",
          ),

        // Last 30 days stats (for consistent data across all components)
        last30DaysCost:
          sql<number>`SUM(CASE WHEN ${usageRecords.date} >= CURRENT_DATE - INTERVAL '30 days' THEN ${usageRecords.totalCost} ELSE 0 END)::numeric`.as(
            "last_30_days_cost",
          ),
        last30DaysTokens:
          sql<number>`SUM(CASE WHEN ${usageRecords.date} >= CURRENT_DATE - INTERVAL '30 days' THEN ${usageRecords.totalTokens} ELSE 0 END)::bigint`.as(
            "last_30_days_tokens",
          ),
        last30DaysInputTokens:
          sql<number>`SUM(CASE WHEN ${usageRecords.date} >= CURRENT_DATE - INTERVAL '30 days' THEN ${usageRecords.inputTokens} ELSE 0 END)::bigint`.as(
            "last_30_days_input_tokens",
          ),
        last30DaysOutputTokens:
          sql<number>`SUM(CASE WHEN ${usageRecords.date} >= CURRENT_DATE - INTERVAL '30 days' THEN ${usageRecords.outputTokens} ELSE 0 END)::bigint`.as(
            "last_30_days_output_tokens",
          ),
        last30DaysCacheCreationTokens:
          sql<number>`SUM(CASE WHEN ${usageRecords.date} >= CURRENT_DATE - INTERVAL '30 days' THEN ${usageRecords.cacheCreationTokens} ELSE 0 END)::bigint`.as(
            "last_30_days_cache_creation_tokens",
          ),
        last30DaysCacheReadTokens:
          sql<number>`SUM(CASE WHEN ${usageRecords.date} >= CURRENT_DATE - INTERVAL '30 days' THEN ${usageRecords.cacheReadTokens} ELSE 0 END)::bigint`.as(
            "last_30_days_cache_read_tokens",
          ),
        last30DaysCredits:
          sql<number>`SUM(CASE WHEN ${usageRecords.date} >= CURRENT_DATE - INTERVAL '30 days' THEN ${usageRecords.credits} ELSE 0 END)::numeric`.as(
            "last_30_days_credits",
          ),
        last30DaysActiveDays:
          sql<number>`COUNT(DISTINCT CASE WHEN ${usageRecords.date} >= CURRENT_DATE - INTERVAL '30 days' THEN ${usageRecords.date} END)::bigint`.as(
  ...
  return await Promise.all([
    ...
    db
      .select({
        ...
        inputTokens: sql<number>`SUM(${usageRecords.inputTokens})::bigint`.as(
          "input_tokens",
        ),
        outputTokens: sql<number>`SUM(${usageRecords.outputTokens})::bigint`.as(
          "output_tokens",
        ),
        cacheCreationTokens:
          sql<number>`SUM(${usageRecords.cacheCreationTokens})::bigint`.as(
        ...
        credits: sql<number>`SUM(${usageRecords.credits})::numeric`.as(
          "credits",
        ),
    ...
    db
      .select({
        ...
        totalCost: sql<number>`${usageRecords.totalCost}::numeric`.as(
          "total_cost",
        ),
        totalTokens: sql<number>`${usageRecords.totalTokens}::bigint`.as(
          "total_tokens",
        ),
        inputTokens: sql<number>`${usageRecords.inputTokens}::bigint`.as(
          "input_tokens",
        ),
        outputTokens: sql<number>`${usageRecords.outputTokens}::bigint`.as(
          "output_tokens",
        ),
        cacheCreationTokens:
          sql<number>`${usageRecords.cacheCreationTokens}::bigint`.as(
            "cache_creation_tokens",
          ),
        cacheReadTokens:
          sql<number>`${usageRecords.cacheReadTokens}::bigint`.as(
            "cache_read_tokens",
          ),
    ...
    db
      .select({
        ...
        lastActiveDate: sql<string>`MAX(${usageRecords.date})`.as(
          "last_active_date",
        ),
        totalRecords: sql<number>`COUNT(${usageRecords.id})`.as(
          "total_records",
        ),
        totalCost: sql<number>`SUM(${usageRecords.totalCost})::numeric`.as(
          "total_cost",
        ...
        agentTypes: sql<
          string[]
        >`ARRAY_AGG(DISTINCT ${usageRecords.agentType})`.as("agent_types"),
  ...
  try {
    ...
    if (!billingStartDate) {
      return NextResponse.json(
        {
          error:
            "CLAUDE_BILLING_CYCLE_START_DATE environment variable is required",
        },
        { status: 500 },
      );
    }
    ...
    const [
      unifiedStats,
      [dailyRecords, deviceDailyRecords, deviceInfo, agentDailyRecords],
    ] = await Promise.all([getUnifiedUsageStats(), getDailyAndDeviceData()]);
    ...
    const totals = {
      ...
      totalCacheCreationTokens: safeNumber(
        statsResult.totalCacheCreationTokens,
      ),
      ...
      avgDailyCost:
        safeNumber(statsResult.activeDays) > 0
          ? safeNumber(statsResult.totalCost) /
            safeNumber(statsResult.activeDays)
          : 0,
    };
    ...
    const last30Days = {
      ...
      totalCacheCreationTokens: safeNumber(
        statsResult.last30DaysCacheCreationTokens,
      ),
      ...
      avgDailyCost:
        safeNumber(statsResult.last30DaysActiveDays) > 0
          ? safeNumber(statsResult.last30DaysCost) /
            safeNumber(statsResult.last30DaysActiveDays)
          : 0,
    };
